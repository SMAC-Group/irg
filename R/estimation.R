...
start_values <- function(root, shoot, del.t) {
  ...
  phi.gm.root <- ar1_to_gm(c(abs(fit.root$coefficients[1]), (summary(fit.root)$sigma)**2), median(del.t))
  psi.root <- fit.root$coefficients[2]
  phi.gm.shoot <- ar1_to_gm(c(abs(fit.shoot$coefficients[1]), (summary(fit.shoot)$sigma)**2), median(del.t))
